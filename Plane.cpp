...
bool Plane::isInside(glm::vec3 pt)
{
	...
	glm::vec3 ua = b-a, ub = c-b, uc = d-c, ud = a-d;  //difference between border points
    glm::vec3 va = pt-a, vb = pt-b, vc = pt-c, vd = pt-d;  //difference between tested point and the border points
	
	
	glm::vec3 c_a = cross(ua, va);
    glm::vec3 c_b = cross(ub, vb);
    glm::vec3 c_c = cross(uc, vc);
    glm::vec3 c_d = cross(ud, vd);
    
    float n_a = glm::dot(c_a, n);
    float n_b = glm::dot(c_b, n); 
    float n_c = glm::dot(c_c, n);
    float n_d = glm::dot(c_d, n);

	return (n_a > 0.0 && n_b > 0.0 && n_c > 0.0 && n_d > 0.0); //point is inbetween all points
...
{
    glm::vec3 n = normalize(cross((b-a), (d-a)));
    return n;
}
